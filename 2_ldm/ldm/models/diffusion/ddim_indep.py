...
Independent guidance variant of DDIM.
"""
...
class IndependentCFGDDIMSampler(object):
    ...
    def sample(
        ...
        independent_guidance=False,
        tissue_conditioning=None,
        uni_tissue_conditioning=None,
        uni_guidance_scale=1.0,
        cohort_guidance_scale=1.0,
        **kwargs,
    ):
        ...
        samples, intermediates = self.ddim_sampling(
            ...
            use_tqdm=use_tqdm,
            independent_guidance=independent_guidance,
            tissue_conditioning=tissue_conditioning,
            uni_tissue_conditioning=uni_tissue_conditioning,
            uni_guidance_scale=uni_guidance_scale,
            cohort_guidance_scale=cohort_guidance_scale,
        )
    ...
    def p_sample_ddim(
        ...
        use_independent = independent_guidance and tissue_conditioning is not None and uni_tissue_conditioning is not None

        if use_independent:
            if sample_uncond_separate:
                e_t_tissue = self.model.apply_model(x, t, tissue_conditioning)
                e_t_uni_tissue = self.model.apply_model(x, t, uni_tissue_conditioning)
                e_t_full = self.model.apply_model(x, t, c)
            else:
                x_in = torch.cat([x] * 3)
                t_in = torch.cat([t] * 3)
                c_in = torch.cat([tissue_conditioning, uni_tissue_conditioning, c])
                e_t_tissue, e_t_uni_tissue, e_t_full = self.model.apply_model(x_in, t_in, c_in).chunk(3)
            e_t = (
                e_t_tissue
                + uni_guidance_scale * (e_t_uni_tissue - e_t_tissue)
                + cohort_guidance_scale * (e_t_full - e_t_uni_tissue)
            )
