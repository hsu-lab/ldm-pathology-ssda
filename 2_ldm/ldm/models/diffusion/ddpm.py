...
class LatentDiffusion(DDPM):
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        ...
        # latent distribution before scaling
        # print(f"[DEBUG] z mean: {z.mean().item():.4f}, std: {z.std().item():.4f}")
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # print(f"[DEBUG p_losses] x_start mean: {x_start.mean():.4f}, std: {x_start.std():.4f}")
        # print(f"[DEBUG p_losses] cond mean: {cond.mean():.4f}, std: {cond.std():.4f}")
        # print(f"[DEBUG p_losses] noise mean: {noise.mean():.4f}, std: {noise.std():.4f}")
        # print(f"[DEBUG p_losses] x_noisy mean: {x_noisy.mean():.4f}, std: {x_noisy.std():.4f}")
        # print(f"[DEBUG p_losses] model_output mean: {model_output.mean():.4f}, std: {model_output.std():.4f}")
    ...
    """
    Olivia 7/17/2025
    The validation loop took too long, and we only care the samples that are used to calculate FID. 
    So similar to FID calculation, only calculate loss based on a subset of validation data. 
    """
    @torch.no_grad()
    def validation_step(self, batch, batch_idx):
        out = self.validation_step_outputs

        # skip everything if 1500 samples already collected
        if len(out) and len(out) * len(out[0]) >= 1500:
            return

        # only compute loss/logging if needed
        # super().validation_step() logs EMA and non-EMA loss
        super().validation_step(batch, batch_idx)
    ...
    def on_validation_epoch_end(self):
        ...
        if not self.real_stats:
            ...
            ## nlst tcga 
            # path = 'analysis/ldm/val_est_fid_stats_with_feats.npz'
            ## tcga cptac 
            # path = 'analysis/ldm/tcga_cptac/val_est_fid_stats_with_feats.npz'
            ## nlst cptac 
            # path = 'analysis/ldm/nlst_cptac/val_est_fid_stats_with_feats.npz'
            ## nlst 
            path = 'analysis/ldm/nlst/val_est_fid_stats_with_feats.npz'
            ## tcga 
            # path = 'analysis/ldm/tcga/val_est_fid_stats_with_feats.npz'
            ## tcga ffpe 
            # path = 'analysis/ldm/tcga_ffpe/val_est_fid_stats_with_feats.npz'
    ...
    def configure_optimizers(self):
        """
        Olivia 10/7/2025 
        Fine-tune cond stage model and cross attention 
        """
        ...
        # # params from the condition encoders 
        # cond_params = []
        # if hasattr(self, "cond_stage_model") and self.cond_stage_model is not None:
        #     # If you want cond encoder trainable; else leave empty
        #     cond_params = list(self.cond_stage_model.parameters())

        # # ONLY condition-injection cross-attn params in UNet 
        # attn2_params = []
        # unet = getattr(self.model, "diffusion_model", None)  # UNetModel
        # if unet is not None:
        #     for n, p in unet.named_parameters():
        #         # Strict option: just the projections that implement cross-attn
        #         if (".attn2.to_q" in n) or (".attn2.to_k" in n) or (".attn2.to_v" in n) or (".attn2.to_out.0" in n):
        #             attn2_params.append(p)

        # # hard-freeze everything else to be safe
        # for p in self.parameters():
        #     p.requires_grad = False
        # for p in itertools.chain(cond_params, attn2_params):
        #     p.requires_grad = True

        # # Build optimizer on the small set
        # params = cond_params + attn2_params
        # opt = torch.optim.AdamW(params, lr=self.learning_rate, weight_decay=1e-2, betas=(0.9, 0.999))
        # return opt
