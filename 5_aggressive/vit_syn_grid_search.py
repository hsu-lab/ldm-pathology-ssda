"""
Fine-tune the huggingface implementation of ViT 
"""
...
class ViTTrainer:
    def __init__(self, num_classes, num_epochs, batch_size, learning_rate, fold_index, 
        ...
        self.image_processor = AutoImageProcessor.from_pretrained(self.hf_model_name, use_fast=True)
    ...
    def start_training(self, train_paths, valid_paths):
        ...
        def focal_per_sample(logits, labels):
            # Your util.FocalLoss must support reduction='none'
        ...
        while early < self.patience and epoch < self.epochs:
            ...
            for step, batch in enumerate(pbar, 1):
                ...
                w = map_weights(is_syn) # weight the per-sample losses 
                loss = (per_s * w).mean() 
                ...
                avg_so_far = running_train / step  # loss per batch
            ...
            # Losses (divide by number of *batches* to mirror your original code)
            ...
            plt.savefig(os.path.join(fig_dir, 'losses.png')); plt.clf()
            ...
            for metric_name in ["accuracy", "sensitivity", "specificity", "f1"]:
                ...
                plt.xlabel('Epoch'); plt.ylabel('Score'); plt.legend(frameon=False, ncol=3)
                plt.savefig(os.path.join(fig_dir, f'{metric_name}.png')); plt.clf()
    ...
    def run_test(self, image_paths, test_name):
        ...
        # Per-class metrics via sklearn (cleaner)
        per_class_f1 = sk_f1(y_true, y_pred, labels=list(range(self.num_classes)),
                             average=None, zero_division=0)

        macro_f1 = sk_f1(y_true, y_pred, labels=list(range(self.num_classes)),
                         average='macro', zero_division=0)
        weighted_f1 = sk_f1(y_true, y_pred, labels=list(range(self.num_classes)),
        ...
        return per_class_f1  # vector of length num_classes
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hf_model_name', type=str, default='google/vit-base-patch16-224-in21k',
                        help='any ViT checkpoint from Hugging Face hub')
    ...
    for syn_weight in [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
        ...
        FIGURE_DIR = os.path.join(exp_folder, 'saved_figures'); os.makedirs(FIGURE_DIR, exist_ok=True)
        MODEL_DIR  = os.path.join(exp_folder, 'saved_models'); os.makedirs(MODEL_DIR, exist_ok=True)
        ...
        for fold_index in range(5):
            ...
            trainer = ViTTrainer(
                ...
                syn_root=syn_root,
                synth_loss_weight=synthetic_weight, # 0.25, # TODO: DOUBLE CHECK THIS!! 
        ...
        logging.info("="*40); logging.info("**Final Summary**"); logging.info("="*40)
        ...
        logging.info(f"Average F1 across folds (CV test) — weighted: {np.nanmean(cv_w):.3f} ({np.nanstd(cv_w):.3f}), "
        ...
        if len(external_test_avg_f1_rows) > 0:
            ...
            logging.info(f"Average F1 across folds (External test) — weighted: {np.nanmean(ext_w):.3f} ({np.nanstd(ext_w):.3f}), "
